...
private:
    ...
    GDALDataset *poOvrDS = nullptr;  // will often be NULL
    int nOvrLevel = 0;
...
public:
    GDALOverviewBand(GDALOverviewDataset *poDSIn, int nBandIn);
    ...
    bThisLevelOnly(bThisLevelOnlyIn)
{
    ...
    // NOTE: we create a fake driver that has the same name as the original
...
int GDALOverviewDataset::CloseDependentDatasets()
{
    ...
    if (poMainDS) {
        for (int i = 0; i < nBands; ++i) {
            GDALOverviewBand* const band = dynamic_cast<GDALOverviewBand*>(papoBands[i]);

            if (band == nullptr) {
                CPLError(CE_Fatal, CPLE_AppDefined, "OverviewBand cast fail.");
                                      ...
                                      GDALRasterIOExtraArg *psExtraArg)
{
    ...
    for (int iBandIndex = 0;
        ...
        GDALOverviewBand *poBand = dynamic_cast<GDALOverviewBand *>(
        ...
        GByte *pabyBandData = static_cast<GByte *>(pData) + iBandIndex *nBandSpace;
...
CPLErr GDALOverviewDataset::GetGeoTransform(double *padfTransform)
{
    ...
    adfGeoTransform[1] *= static_cast<double>(poMainDS->GetRasterXSize()) / nRasterXSize;
    adfGeoTransform[2] *= static_cast<double>(poMainDS->GetRasterYSize()) / nRasterYSize;
    adfGeoTransform[4] *= static_cast<double>(poMainDS->GetRasterXSize()) / nRasterXSize;
    adfGeoTransform[5] *= static_cast<double>(poMainDS->GetRasterYSize()) / nRasterYSize;
...
{
    if (poUnderlyingBand && bForceOpen)
